...
class Aplicacion():
    ...
    def __init__(self):
        ...
        self.__p_d = StringVar()
        self.__p_p = StringVar()
        self.dolaresEntry = ttk.Entry(mainframe, width=10, textvariable=self.__p_d)
        self.dolaresEntry.grid(column=2, row=1, sticky=(W, E))
        ttk.Label(mainframe, textvariable=self.__p_p).grid(column=2, row=2, sticky=(W, E))
    ...
    def convertir(self):
        try:
            ...
            cotizacion = self.cotiz()
            precio_pesos = precio_dolares * cotizacion
            self.__p_p.set(f"{precio_pesos:.2f}")
        except ValueError:
